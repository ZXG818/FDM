  ...
  double dx, double dy)
{
  ...
  for (i = 2; i < nx - 2; i++)
  {
    for (j = 2; j < ny - 2; j++)
    {
      ...
      if (last_u[i*ny + j].value>0)
      {
        if (last_v[i*ny + j].value>0)
        {
          // velocity parameters
        ...
        else if (last_v[i*ny + j].value < 0)
        {
          // velocity parameters
          u[i*ny + j].params.AP = (flow_u * 3 / (2 * dx)) -
            (flow_v * 3 / (2 * dy)) +
            (2 * dynamic_viscosity / (dx*dx)) +
            (2 * dynamic_viscosity / (dy*dy));
          u[i*ny + j].params.AW = (flow_u * 4 / (2 * dx)) + dynamic_viscosity / (dx*dx);
          u[i*ny + j].params.AWW = (-flow_u / (2 * dx));
          u[i*ny + j].params.AE = dynamic_viscosity / (dx*dx);
          u[i*ny + j].params.AN = (-flow_v * 4 / (2 * dy)) + dynamic_viscosity / (dy*dy);
          u[i*ny + j].params.ANN = flow_v / (2 * dy);
          u[i*ny + j].params.AS = dynamic_viscosity / (dy*dy);
          u[i*ny + j].params.ASS = 0.0;
          u[i*ny + j].params.AEE = 0.0;
        }
        // pressure parameters
      ...
      else if (last_u[i*ny + j].value < 0)
      {
        if (last_v[i*ny + j].value>0)
        {
          ...
          u[i * ny + j].params.AP = (-3 * flow_u / (2 * dx)) +
            ...
            2 * dynamic_viscosity / (dx * dx) +
            2 * dynamic_viscosity / (dy * dy);
          u[i * ny + j].params.AE = (-4 * flow_u / (2 * dx)) + dynamic_viscosity / (dx * dx);
          u[i * ny + j].params.AEE = flow_u / (2 * dx);
          u[i * ny + j].params.AW = dynamic_viscosity / (dx * dx);
          u[i * ny + j].params.AS = (4 * flow_v / (2 * dy)) + dynamic_viscosity / (dy * dy);
          u[i * ny + j].params.ASS = -flow_v / (2 * dy);
          u[i * ny + j].params.AN = dynamic_viscosity / (dy * dy);
          u[i * ny + j].params.ANN = 0.0;
          u[i * ny + j].params.AWW = 0.0;
        }
        else if (last_v[i*ny + j].value < 0)
        {
          // velocity parameters
          u[i*ny + j].params.AP = (-flow_u * 3 / (2 * dx)) -
            ...
            (2 * dynamic_viscosity / (dx*dx)) +
            (2 * dynamic_viscosity / (dy*dy));
          u[i*ny + j].params.AE = (-flow_u * 4 / (2 * dx)) + dynamic_viscosity / (dx*dx);
          u[i*ny + j].params.AEE = flow_u / (2 * dx);
          u[i*ny + j].params.AW = dynamic_viscosity / (dx*dx);
          u[i*ny + j].params.AN = (-flow_v * 4 / (2 * dy)) + dynamic_viscosity / (dy*dy);
          u[i*ny + j].params.ANN = flow_v / (2 * dy);
        ...
        // pressure parameters
        u[i*ny + j].pressure_params.AP = 3 / (2 * dx);
        u[i*ny + j].pressure_params.AE = -4 / (2 * dx);
        u[i*ny + j].pressure_params.AEE = 1 / (2 * dx);
        u[i*ny + j].pressure_params.AW = 0.0;
        u[i*ny + j].pressure_params.AWW = 0.0;
        u[i*ny + j].pressure_params.AS = 0.0;
        u[i*ny + j].pressure_params.ASS = 0.0;
        u[i*ny + j].pressure_params.AN = 0.0;
        u[i*ny + j].pressure_params.ANN = 0.0;
      }
    }
  }
}
